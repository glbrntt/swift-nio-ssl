...
func main() throws {
  ...
  let certificateData = Data(try certificate.toDERBytes())
  try certificateData.write(to: certificateURL)

  let privateKeyURL = URL(fileURLWithPath: "key.der")
  let privateKeyData = try privateKey.withUnsafeDERBuffer { buffer in
    return Data(buffer)
  }
